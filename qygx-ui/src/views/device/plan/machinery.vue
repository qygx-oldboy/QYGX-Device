<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="计划时间" prop="firstMaintainTime">
...
} from "@/api/mes/dv/checkmachinery";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      this.$refs.machinerySelect.queryParams.deviceType = this.deviceType;
      this.$refs.machinerySelect.handleQuery();
    },
    //设备资源选择回调
    onMachineryAdd(rows) {
      if (rows != null && rows.length > 0) {
        rows.forEach((row) => {
          ...
          const data = {
            planId: row.planId,
            machineryId: row.deviceId,
            machineryCode: row.deviceCode,
            machineryName: row.name,
            machineryBrand: row.manufacturer,
          };
          addCheckmachinery(data).then((response) => {
            this.getList();
          });
        });
      }
    ...
    //选择回调
    onMachinerySubmit() {
      if (this.checkmachineryList != null && this.checkmachineryList.length > 0) {
        this.checkmachineryList.forEach((row) => {
               updateCheckmachinery(row).then((response) => {
                this.getList();
              });
