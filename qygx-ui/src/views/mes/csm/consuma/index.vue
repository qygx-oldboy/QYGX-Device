    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['mes:csm:consuma:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['mes:csm:consuma:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['mes:csm:consuma:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="consumaList" @selection-change="handleSelectionChange">
  ...
  methods: {
    ...
    getList() {
      ...
      listConsuma(this.queryParams).then(response => {
        this.consumaList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getConsuma(consumaId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.consumaId != null) {
            updateConsuma(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addConsuma(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除备件档案编号为"' + consumaIds + '"的数据项？').then(function() {
        return delConsuma(consumaIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('mes:csm/consuma/export', {
        ...this.queryParams
      }, `consuma_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
